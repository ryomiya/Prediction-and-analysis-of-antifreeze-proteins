...
plt.legend()
plt.title('ROC curve (Dataset S)')
plt.xlabel('FPR: False positive rate')
plt.ylabel('TPR: True positive rate')
plt.grid(True)
plt.rcParams["font.size"] = 12
plt.savefig('ROC_DatasetS.png')
...
np.savetxt('Fimportance_DatasetS.csv', np.sort(f_importance2)[::-1], delimiter=',')

arg = np.argsort(f_importance)[::-1]

np.savetxt('Fimportance_arg_DatasetS.csv', arg.astype(np.uint64), delimiter=',')
...
with open('DatasetS_evidence.txt', 'a') as f:
    f.writelines(li)

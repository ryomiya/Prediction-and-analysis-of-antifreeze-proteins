...
plt.legend()
plt.title('ROC curve (Dataset R)')
plt.xlabel('FPR: False positive rate')
plt.ylabel('TPR: True positive rate')
plt.grid(True)
plt.rcParams["font.size"] = 12
plt.savefig('ROC_DatasetR.png')
...
np.savetxt('Fimportance_DatasetR.csv', np.sort(f_importance2)[::-1], delimiter=',')

arg = np.argsort(f_importance)[::-1]

np.savetxt('Fimportance_arg_DatasetR.csv', arg.astype(np.uint64), delimiter=',')
...
with open('DatasetR_evidence.txt', 'a') as f:
    f.writelines(li)

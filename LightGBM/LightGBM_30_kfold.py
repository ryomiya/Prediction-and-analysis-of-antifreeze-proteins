...
df = pd.read_csv("all_cdhit30.csv")
...
plt.legend()
plt.title('ROC curve (Dataset 30)')
plt.xlabel('FPR: False positive rate')
plt.ylabel('TPR: True positive rate')
plt.grid(True)
plt.rcParams["font.size"] = 12
plt.savefig('ROC_Dataset30.png')
...
np.savetxt('Fimportance_Dataset30.csv', np.sort(f_importance2)[::-1], delimiter=',')

arg = np.argsort(f_importance)[::-1]

np.savetxt('Fimportance_arg_Dataset30.csv', arg.astype(np.uint64), delimiter=',')
...
with open('Dataset30_evidence.txt', 'a') as f:
    f.writelines(li)
